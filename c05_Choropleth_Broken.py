...
geodf = mainPath + "iData\\11_SHPs\\PLDZones_GOR.geojson"
...
district_options = [{"label": value, "value": value} for value in df["ZoneID"].unique()]
# scenario options
scenario_options = [
...
def origin_map_update(
    ...
    if button_id == "submit-button" and submit_button:
        ...
        # update map with the new filtered data
        fig = pFunc.set_map(df_new, geodf, layer_value, dropdown_value, "choropleth")
